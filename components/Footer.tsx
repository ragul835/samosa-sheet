  ...
  return (
    <footer className="bg-leaf-900 py-12 text-green-50">
      <div className="container-shell grid gap-8 md:grid-cols-2 lg:grid-cols-4">
        ...
        <div><h3 className="font-bold text-white">Contact</h3><div className="mt-3 flex flex-col items-start gap-2 text-sm text-green-100"><a href={`tel:${site.phone}`}>{site.phoneDisplay}</a><OrderWhatsAppButton className="text-left hover:text-white">Order on WhatsApp</OrderWhatsAppButton><a href={`mailto:${site.email}`}>{site.email}</a><span>{site.address}</span></div></div>
